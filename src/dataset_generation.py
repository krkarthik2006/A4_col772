...
def generate_and_parse_batch_with_logprobs(
        ...
        *,
        ...
        top_p: float,
        top_k: int,
) -> tuple[list[dict[str, str]], list[list]]:
    """Like generate_and_parse_batch but also returns per-token top-K log-probs."""
    raw_generations, all_logprobs = prompt_vllm_with_logprobs(
        model,
        tokenizer,
        batch_messages=_build_batch_messages(prompts),
        top_k=top_k,
    ...
    return [_parse_teacher_generation(text) for text in raw_generations], all_logprobs
...
def main() -> None:
    ...
    with output_path.open("w", encoding="utf-8") as fp:
        ...
        for batch_rows in tqdm(
            ...
            if top_k_logits > 0:
                parsed_batch, batch_logprobs = generate_and_parse_batch_with_logprobs(
                    teacher,
                    tokenizer,
                    prompts,
                    max_new_tokens=max_new_tokens,
            ...
            for row, question_with_choices, prompt, parsed, token_logprobs in zip(
                batch_rows, questions_with_choices, prompts, parsed_batch, batch_logprobs
            ):
